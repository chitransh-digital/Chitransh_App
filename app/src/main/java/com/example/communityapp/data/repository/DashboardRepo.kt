...
class DashboardRepo @Inject constructor(private val db: FirebaseFirestore, private val storage: FirebaseStorage, private val api: CustomAPI) {
    ...
    suspend fun updateMember(member: addMember, familyHash : String, memberHash : String) =
        api.updateMember(member, familyHash, memberHash)

    suspend fun getNewFeeds(limit: Int, page :Int) = api.getFeeds(limit,page)
    
    suspend fun deleteMember(familyHash : String, memberHash : String) = api.deleteMember(familyHash, memberHash)
